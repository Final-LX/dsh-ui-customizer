...
- **组件**：圆角 0–24px + 阴影层级（无/轻/标准/强）+ 边框风格（无/细线/相框/双线/发光，作用于输入框/侧边栏/对话框）
...
| | 边框风格 | 无/细线/相框/双线/发光，作用于输入框、侧边栏、对话框 |
...
- 「边框风格」用侧边栏根节点类名 `.hHd-Xa_root`（`@deepseek-ai/dsh-client-ui-sidebar` 的 CSS Modules 哈希，
  随 DSH 版本变化），升级后若侧边栏边框失效，去 `dsh-client-ui-sidebar/lib/client.js` 核对最新类名并更新 `FRAME_SELECTOR`。
